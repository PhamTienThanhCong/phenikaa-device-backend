...
def create_user(current_user: Annotated[UserBase, Depends(get_current_active_user)], user: CustomerBase, db: Session = Depends(get_db)):
...
def update_user(current_user: Annotated[UserBase, Depends(get_current_active_user)], id: int, user: CustomerUpdate, db: Session = Depends(get_db)):
...
def delete_user(current_user: Annotated[UserBase, Depends(get_current_active_user)], id: int, db: Session = Depends(get_db)):
  if not customer_service.get_user_by_id(db, id):
    raise HTTPException(status_code=404, detail="User not found")
