...
from app.services.devices.device_schema import DeviceCoreSchema, DeviceCreateResponse, DeviceCreateSchema, DeviceCreateSchemaFull, DeviceUpdateResponse, DeviceUpdateSchema, DeviceUpdateSchemaFull
...
def upload_image_presigned_url(presigned_url_id: str, file: UploadFile ,db: Session = Depends(get_db)):
...
def delete_device(current_user: Annotated[UserBase, Depends(get_current_active_user)], device_id: int, db: Session = Depends(get_db)):
